//
//  SignInContentView.swift
//  MovieAppDemo
//
//  Created by h.yamaguchi on 2024/09/17.
//

import SwiftUI

struct RefreshChecker {
    init() {
        print("refreshed!")
    }
}

struct SignInContentView: View {
    @EnvironmentObject var store: ReduxStore<AppState>
    var state: SignInPageState { store.state.routingState.signInPageState }
    let actionCreator: SignInPageStateActionCreator = ActionCreatorAssembler().resolve()
    
    var body: some View {
        ZStack() {
            VStack(spacing: 0.0) {
                Text("Demo App")
                    .fontWeight(.bold)
                    .font(.largeTitle)
                
                VStack(alignment: .leading) {
                    Text("Username")
                        .font(.footnote)
                        .padding(.horizontal, 32)
                    TextField("Username", text: Binding(get: {
                        state.userName
                    }, set: { value, _ in
                        Task {
                            await store.dispatch(SignInPageStateAction.updateUserName(value))
                        }
                    }))
                    .textFieldStyle(.plain)
                    .padding(.horizontal, 5)
                    .padding(.vertical, 5)
                    .background(RoundedRectangle(cornerRadius: 4).fill(Color.Background.textField))
                    .padding(.horizontal, 32)
                    
                    Spacer().frame(height: 20)
                    Text("Password")
                        .font(.footnote)
                        .padding(.horizontal, 32)
                    SecureField("Password", text: Binding(get: {
                        state.password
                    }, set: { value, _ in
                        Task {
                            await store.dispatch(SignInPageStateAction.updatePassword(value))
                        }
                    }))
                    .textFieldStyle(.plain)
                    .padding(.horizontal, 5)
                    .padding(.vertical, 5)
                    .background(RoundedRectangle(cornerRadius: 4).fill(Color.Background.main))
                    .padding(.horizontal, 32)
                }
                .frame(height: 200)
                
                PrimaryButton(title: "SignIn (dummy)") {
                    Task {
                        await store.dispatch(actionCreator.signIn())
                    }
                }
                .padding(.horizontal, 32)
            }
            .frame(maxWidth: .infinity, maxHeight: .infinity)
            .background(Color.Background.main)
            
            CircularProgressBar(progress: Binding(
                get: { CGFloat(state.progress) },
                set: { value, _ in
                    Task {
                        await store.dispatch(SignInPageStateAction.updateProgress(Float(value)))
                    }
                }), color: Color(red: 138 / 255, green: 111 / 255, blue: 245 / 255, opacity:1.0))
                .frame(width: 80, height: 80)
                .hidden(!state.showProgress)
        }
    }
}

#Preview {
    SignInContentView()
        .environmentObject(store)
}
